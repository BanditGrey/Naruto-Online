package Processors.Game.Lobby.Challenge.Compoents
{
   ...
   public class THeroList extends TUIComponent
   {
      ...
      protected var FTakeBackHero:Function;
      ...
      protected function InitHeroList() : void
      {
         ...
         while(_loc1_ < MAX_COUNT)
         {
            ...
            _loc2_.BTN_Get.addEventListener(MouseEvent.CLICK,this.OnTakeBackHero);
      ...
      protected function OnSelectHero(param1:MouseEvent) : void
      {
         ...
         {
            _loc3_ = this.FHerosData.GetHeroByIndex(_loc2_);
            if(!_loc3_.IsRecommand || _loc3_.IsInHostel)
            {
               return;
            }
            if(_loc3_ != null)
      ...
      protected function OnTakeBackHero(param1:MouseEvent) : void
      {
         var _loc2_:int = 0;
         var _loc3_:THero = null;
         _loc2_ = int(String(param1.currentTarget.parent.name).slice(8));
         if(this.FTakeBackHero != null)
         {
            _loc3_ = this.FHerosData.GetHeroByIndex(_loc2_);
            this.FTakeBackHero(_loc3_);
         }
      }
      ...
      public function get TakeBackHero() : Function
      {
         return this.FTakeBackHero;
      }
      
      public function set TakeBackHero(param1:Function) : void
      {
         this.FTakeBackHero = param1;
      }
      ...
      public function UpdataHeroList() : void
      {
         ...
         while(_loc1_ < MAX_COUNT)
         {
            ...
            if(_loc1_ < this.FHerosData.Count)
            {
               ...
               _loc2_.MC_Recommand.visible = _loc3_.IsRecommand;
               if(_loc3_.IsRecommand)
               {
                  _loc2_.MC_Recommand.visible = true;
                  _loc2_.filters = [];
               }
               else
               {
                  _loc2_.MC_Recommand.visible = false;
                  _loc2_.filters = [TGameUtil.GaryColorFilters];
               }
               _loc2_.mc_outFight.visible = Boolean(_loc3_.FightPosition > 0);
               _loc2_.BTN_Get.visible = _loc3_.IsInHostel;
