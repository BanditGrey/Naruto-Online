package Processors.Game.Lobby.Exercise.ConsumeRank
{
   ...
   import Processors.Game.Lobby.Exercise.ConsumeRank.Compoents.TUIRankReward;
   ...
   public class TProcessorWindowTotalRank extends TProcessorLobbyWindow
   {
      ...
      protected function UpdateReward() : void
      {
         ...
         {
            _loc2_ = this.FConsumeRank.RankRewardList.length;
            _loc1_ = 0;
            while(_loc1_ < _loc2_)
            {
      ...
      protected function UpdateRank() : void
      {
         ...
         this.FUIPage.TotalQuantity = this.FConsumeRank.RankInfoList.length;
         this.FUIPage.Update();
         _loc1_ = 0;
         while(_loc1_ < MAX_COUNT)
         {
            ...
            if(_loc2_ < this.FConsumeRank.RankInfoList.length)
            {
               _loc4_.visible = true;
               _loc5_.visible = true;
               _loc6_.visible = true;
               _loc7_.visible = true;
               _loc4_.text = this.FConsumeRank.RankInfoList[_loc2_].Rank.toString();
               _loc5_.text = this.FConsumeRank.RankInfoList[_loc2_].ServerName;
               _loc6_.text = this.FConsumeRank.RankInfoList[_loc2_].UserName;
               _loc7_.text = this.FConsumeRank.RankInfoList[_loc2_].Score.toString();
            }
            else
            {
         ...
         this.FShowItem.UpdateUI(this.FConsumeRank.GoldShowItems);
      }
      
      protected function UpdateText() : void
      {
         this.FTF_CurScore.text = TUtilityString.Format(STRING_CONSUMERANK.FORMAT_CUR_SCORE,this.FConsumeRank.PerScore);
         if(this.FConsumeRank.PerRank == 1)
         {
            this.FTF_DiffScore.visible = false;
         }
         else if(this.FConsumeRank.PerRank > RANK_COUNT)
         {
            this.FTF_DiffScore.visible = true;
            this.FTF_DiffScore.text = TUtilityString.Format(STRING_CONSUMERANK.FORMAT_DIFF_FIFTY_SCORE,this.FConsumeRank.DiffScore);
         }
         else
         {
            this.FTF_DiffScore.visible = true;
            this.FTF_DiffScore.text = TUtilityString.Format(STRING_CONSUMERANK.FORMAT_DIFF_SCORE,this.FConsumeRank.DiffScore);
         }
         if(this.FConsumeRank.PerRank > RANK_COUNT)
         {
            this.FTF_Rank.text = STRING_CONSUMERANK.FORMAT_CUR_RANK;
         }
         else
         {
            this.FTF_Rank.text = this.FConsumeRank.PerRank.toString();
         }
         if(this.FConsumeRank.CurReturn > 0)
         {
            this.FMC_Scene.TF_CurReturn.text = this.FConsumeRank.CurReturn + "%";
         }
         else
         {
            this.FMC_Scene.TF_CurReturn.text = STRING_CONSUMERANK.FORMAT_NONE_RATE;
         }
         if(this.FConsumeRank.DiffGold > 0)
         {
            this.FMC_Scene.TF_NextReturn.text = TUtilityString.Format(this.FConsumeRank.Desc4,this.FConsumeRank.DiffGold,this.FConsumeRank.NextReturn);
         }
         else
         {
      ...
      protected function ProcessorOnNameUp(param1:MouseEvent) : void
      {
         ...
         {
            this.FOnNameUp(this.FConsumeRank.RankInfoList[_loc3_].Identify0,this.FConsumeRank.RankInfoList[_loc3_].Identify1);
         }
      }
      ...
      protected function ProcessorOnShowEquipDesc(param1:uint, param2:int = 0) : void
      {
         ...
         {
            this.FOnShowEquip(this.FConsumeRank.GoldEquipments);
         }
      }
