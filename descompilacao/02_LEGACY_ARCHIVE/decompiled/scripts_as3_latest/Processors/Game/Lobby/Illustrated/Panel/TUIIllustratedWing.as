package Processors.Game.Lobby.Illustrated.Panel
{
   ...
   public class TUIIllustratedWing extends TUIBaseWindow
   {
      ...
      public function TUIIllustratedWing(param1:TUIComponent)
      {
         super(param1);
      }
      ...
      {
         TIllustratedModel.Wings = TIllustratedModel.Sort(TIllustratedModel.Wings);
         if(this._maxPage == -1)
         {
            this._maxPage = Math.ceil(TIllustratedModel.Wings.length / 1);
            this.page = 1;
         }
         else
         {
      ...
      override public function LogicsPerform() : void
      {
         ...
         super.LogicsPerform();
         var _loc1_:int = Math.max((this._page - 1) * 1,0);
         var _loc2_:int = Math.min(this._page * 1,TIllustratedModel.Wings.length);
         var _loc3_:int = 0;
         while(_loc3_ < 1)
         {
            if(_loc1_ + _loc3_ < _loc2_)
            {
               _loc4_ = TIllustratedModel.Wings[_loc1_ + _loc3_];
               TGameUtil.ShowImageByID(TGameUtil.Type_Wing,this._psychics[_loc3_],CONST_MODULES.MODULE_Illustrated,_loc4_.SuitIdVector[0]);
            }
            _loc3_++;
         }
      }
      ...
      public function set page(param1:int) : void
      {
         ...
         var _loc2_:int = Math.max((this._page - 1) * 1,0);
         var _loc3_:int = Math.min(this._page * 1,TIllustratedModel.Wings.length);
         var _loc4_:int = 0;
         while(_loc4_ < 1)
         {
            ...
            {
               _loc5_ = TIllustratedModel.Wings[_loc2_ + _loc4_];
