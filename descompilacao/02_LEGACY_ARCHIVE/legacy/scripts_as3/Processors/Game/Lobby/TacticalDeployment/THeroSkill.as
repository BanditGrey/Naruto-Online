package Processors.Game.Lobby.TacticalDeployment
{
   ...
   public class THeroSkill extends TUIComponent
   {
      ...
      public function THeroSkill(param1:TUIComponent, param2:MovieClip)
      {
         ...
         this.FSkillsAwaken = new TSkills();
      }
      ...
      protected function OnSkillSelect(param1:MouseEvent) : void
      {
         ...
         _loc4_ = SLogicsCore.Character.MainHeroSelectedSkill;
         if(this.FSkillType == SKILL_TYPE_NORMAL)
         {
            if(_loc4_.Identifier == this.FSkills.GetSkillByIndex(0).Identifier)
            {
         ...
         {
            if(_loc4_.Identifier == this.FSkillsAwaken.GetSkillByIndex(0).Identifier)
            {
               _loc2_ += (this.FCurPage - 1) * MAX_COUNT + 1;
            }
            else
            ...
            _loc3_ = this.FSkillsAwaken.GetSkillByIndex(_loc2_);
         }
         if(_loc3_ != null)
         {
      ...
      protected function OnCurSkillMove(param1:MouseEvent) : void
      {
         ...
         _loc2_ = SLogicsCore.Character.MainHeroSelectedSkill;
      ...
      public function SetSkillData(param1:TSkills, param2:int) : void
      {
         if(param1 == null)
         {
            return;
         ...
         param1.Sort();
         this.SetSkill(param1);
         this.FSkillType = param2;
         if(this.FCurPage == 0)
         {
            this.FCurPage = 1;
         }
         ...
         {
            this.FTotlePage = Math.max(int(this.FSkillsAwaken.Count - 1 - 1) / MAX_COUNT + 1,1);
         }
         this.UpdataUI();
         this.CheckBtn();
      }
      
      public function SetSkill(param1:TSkills) : void
      {
         var _loc2_:int = 0;
         var _loc3_:TSkill = null;
         this.FSkills.Clear();
         this.FSkillsAwaken.Clear();
         _loc2_ = 0;
         while(_loc2_ < param1.Count)
         {
            _loc3_ = param1.GetSkillByIndex(_loc2_);
            if(_loc3_.Type == TSkill.SKILL_TYPE_AWAKEN)
            {
               this.FSkillsAwaken.Add(_loc3_);
            }
            else
            {
               this.FSkills.Add(_loc3_);
            }
            _loc2_++;
         }
      }
      ...
      public function UpdataUI() : void
      {
         ...
         _loc6_ = SLogicsCore.Character.MainHeroSelectedSkill;
         this.SkillConfig = this.FSkillBins.GetDatebaseByIdentifier(_loc6_.Identifier) as TSkillConfig;
         ...
         _loc4_.tf_name.text = String(_loc6_.Name);
         _loc1_ = 0;
         while(_loc1_ < MAX_COUNT)
         {
            ...
            {
               _loc5_ = this.FSkills;
               if(_loc6_.Identifier == this.FSkills.GetSkillByIndex(0).Identifier)
               {
                  _loc2_ = (this.FCurPage - 1) * MAX_COUNT + _loc1_ + 1;
               }
               else
               {
                  _loc2_ = (this.FCurPage - 1) * MAX_COUNT + _loc1_;
               }
            }
            else
            {
               _loc5_ = this.FSkillsAwaken;
               if(this.FSkillsAwaken.Count == 0)
               {
                  _loc2_ = 999;
               }
               else if(_loc6_.Identifier == this.FSkillsAwaken.GetSkillByIndex(0).Identifier)
               {
                  _loc2_ = (this.FCurPage - 1) * MAX_COUNT + _loc1_ + 1;
               }
               else
            ...
            if(_loc2_ < _loc5_.Count)
            {
               _loc4_.visible = true;
               _loc3_ = _loc5_.GetSkillByIndex(_loc2_);
