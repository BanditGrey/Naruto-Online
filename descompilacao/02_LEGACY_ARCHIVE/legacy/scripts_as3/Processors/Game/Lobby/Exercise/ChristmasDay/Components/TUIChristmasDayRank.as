package Processors.Game.Lobby.Exercise.ChristmasDay.Components
{
   ...
   public class TUIChristmasDayRank extends TProcessorLobbyWindow
   {
      
      protected static const SIZE_Window_Width:uint = 851;
      ...
      protected static const SPECIAL_BOX_COUNT:int = 2;
      
      protected static const RANK_COUNT:int = 12;
      
      protected var FChristmasDay:TChristmasDay;
      ...
      protected var FUISpecialVect:Vector.<TUIBaseBox>;
      ...
      public function TUIChristmasDayRank(param1:TUIComponent)
      {
         super(param1);
         this.FChristmasDay = SLogicsCore.ChristmasDay;
         this.FUIRankBoxVect = new Vector.<TUIBaseBox>(RANK_LEVEL);
         this.FUISpecialVect = new Vector.<TUIBaseBox>(RANK_LEVEL);
      }
      ...
      override protected function ResourcesPerform_UIDispatch() : void
      {
         ...
         this.FMC_Scene = TUtilityReflection.CreateDisplayObjectInstance("MC_ChristmasDayRank") as MovieClip;
         ...
         {
            _loc3_ = new TUIBaseBox(this,RANK_BOX_COUNT);
            _loc3_.Perform_UIDispatch(this.FMC_Scene["MC_Box" + _loc1_]);
            _loc3_.OnOverlay = this.SlotsOnOver;
            _loc3_.OnOut = this.SlotsOnOut;
            _loc3_.TitleHintOnOver = this.MCTitleEffectOnOver;
            _loc3_.TitleHintOnOut = this.MCTitleEffectOnOut;
            this.FUIRankBoxVect[_loc1_] = _loc3_;
            _loc1_++;
         }
         _loc1_ = 0;
         while(_loc1_ < RANK_LEVEL)
         {
            _loc3_ = new TUIBaseBox(this,SPECIAL_BOX_COUNT);
            _loc3_.Perform_UIDispatch(this.FMC_Scene["MC_Special" + _loc1_]);
            _loc3_.OnOverlay = this.SlotsOnOver;
            _loc3_.OnOut = this.SlotsOnOut;
            this.FUISpecialVect[_loc1_] = _loc3_;
            _loc1_++;
         }
      ...
      {
         if(this.FChristmasDay.CurMyRank <= 0)
         {
            this.FMC_Scene["TF_CurRank"].text = STRING_BASEACTIVITY.FORMAT_NEVER_IN_RANK;
         }
         else
         {
            this.FMC_Scene["TF_CurRank"].text = this.FChristmasDay.CurMyRank.toString();
         }
         this.FMC_Scene["TF_Count"].text = this.FChristmasDay.RankPoint.toString();
      }
      ...
      protected function UpdateBox() : void
      {
         ...
         while(_loc1_ < RANK_LEVEL)
         {
            if(_loc1_ < this.FChristmasDay.RankGiftList.length)
            {
               _loc7_ = this.FChristmasDay.RankGiftList[_loc1_];
               _loc3_ = _loc7_.Inventories;
               this.FUIRankBoxVect[_loc1_].UpdateUI(_loc3_);
               this.FUIRankBoxVect[_loc1_].UpdateTitleEffect(_loc7_.TitleID);
               _loc4_ = TUtilityString.Format(STRING_BASEACTIVITY.FORMAT_RANK_VALUE,_loc7_.Min,_loc7_.Max);
               this.FUIRankBoxVect[_loc1_].SetDescText(0,_loc4_);
            }
            _loc1_++;
         }
      }
      
      protected function UpdateSpecial() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:TInventories = null;
         ...
         while(_loc1_ < RANK_LEVEL)
         {
            if(_loc1_ < this.FChristmasDay.RankSpecialList.length)
            {
               _loc3_ = this.FChristmasDay.RankSpecialList[_loc1_];
               this.FUISpecialVect[_loc1_].UpdateUI(_loc3_);
               _loc5_ = this.FChristmasDay.ScoreCommand[_loc1_].toString();
               this.FUISpecialVect[_loc1_].SetDescText(0,_loc5_);
            }
            _loc1_++;
         }
      }
      ...
      protected function UpdateRank() : void
      {
         ...
         this.FUIPage.TotalQuantity = this.FChristmasDay.RankPlayerList.length;
         this.FUIPage.Update();
         _loc1_ = 0;
         while(_loc1_ < RANK_COUNT)
         {
            ...
            if(_loc2_ < this.FChristmasDay.RankPlayerList.length)
            {
               _loc8_ = this.FChristmasDay.RankPlayerList[_loc2_];
               if(this.FCurPage == 0)
               {
      ...
      override protected function LogicsPerform() : void
      {
         ...
         if(this.visible)
         {
            ...
            while(_loc1_ < RANK_LEVEL)
            {
               if(this.FUISpecialVect[_loc1_])
               {
                  this.FUISpecialVect[_loc1_].LogicsPerform();
               }
               _loc1_++;
            }
         }
