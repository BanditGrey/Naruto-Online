package Logics.DatebaseVO.VO
{
   import Foundation.Resources.Spaces.ResourcesSpace;
   import Foundation.Utilities.TUtilityString;
   import Logics.DatebaseVO.TDatebaseVO;
   import flash.utils.ByteArray;
   
   use namespace ResourcesSpace;
   
   public class TNarutoRoadDayTask extends TDatebaseVO
   {
      
      protected var FName:String;
      
      protected var FTips:String;
      
      protected var FAccording:uint;
      
      protected var FDescription:String;
      
      protected var FAccept:uint;
      
      protected var FIsgoto:uint;
      
      protected var FPicture:uint;
      
      public function TNarutoRoadDayTask()
      {
         super();
      }
      
      override ResourcesSpace function ReadDataByXml(param1:XML) : void
      {
         var _loc2_:XML = null;
         var _loc3_:String = null;
         var _loc4_:* = undefined;
         var _loc5_:uint = 0;
         var _loc6_:int = 0;
         _loc5_ = uint(param1.elements().length());
         _loc6_ = 0;
         while(_loc6_ < _loc5_)
         {
            _loc2_ = param1.elements()[_loc6_];
            _loc3_ = String(_loc2_.name());
            _loc4_ = _loc2_;
            if(_loc3_ == "id")
            {
               Coerce(uint(_loc4_));
            }
            else
            {
               _loc3_ = "F" + _loc3_;
               if(hasOwnProperty(_loc3_))
               {
                  this[_loc3_] = _loc4_;
               }
               else
               {
                  if(Count_AttributeName.indexOf(_loc3_.slice(1,2)) < 0)
                  {
                     _loc3_ = "F" + _loc3_.slice(1,2).toLocaleUpperCase() + _loc3_.slice(2);
                  }
                  if(hasOwnProperty(_loc3_.slice(1)))
                  {
                     if(this[_loc3_] is Boolean)
                     {
                        this[_loc3_] = Boolean(int(_loc4_));
                     }
                     else
                     {
                        this[_loc3_] = _loc4_;
                     }
                  }
               }
            }
            _loc6_++;
         }
      }
      
      override ResourcesSpace function WriteDataToStream(param1:ByteArray) : void
      {
         param1.writeUnsignedInt(FIdentifier);
         TUtilityString.FlushUTF(param1,this.FName);
         TUtilityString.FlushUTF(param1,this.FTips);
         param1.writeUnsignedInt(this.FAccording);
         TUtilityString.FlushUTF(param1,this.FDescription);
         param1.writeUnsignedInt(this.FAccept);
         param1.writeUnsignedInt(this.FIsgoto);
         param1.writeUnsignedInt(this.FPicture);
      }
      
      override ResourcesSpace function ReadDataByStream(param1:ByteArray) : void
      {
         this.FName = TUtilityString.FetchUTF(param1);
         this.FTips = TUtilityString.FetchUTF(param1);
         this.FAccording = param1.readUnsignedInt();
         this.FDescription = TUtilityString.FetchUTF(param1);
         this.FAccept = param1.readUnsignedInt();
         this.FIsgoto = param1.readUnsignedInt();
         this.FPicture = param1.readUnsignedInt();
      }
      
      public function get Name() : String
      {
         return this.FName;
      }
      
      public function get Tips() : String
      {
         return this.FTips;
      }
      
      public function get According() : uint
      {
         return this.FAccording;
      }
      
      public function get Description() : String
      {
         return this.FDescription;
      }
      
      public function get Accept() : uint
      {
         return this.FAccept;
      }
      
      public function get Isgoto() : uint
      {
         return this.FIsgoto;
      }
      
      public function get Picture() : uint
      {
         return this.FPicture;
      }
   }
}

