package Processors.Game.Lobby.Exercise.Cornucopia
{
   ...
   public class TProcessorDecActiveMySelected extends TProcessorLobbyWindow
   {
      ...
      override protected function ResourcesPerform_UIDispatch() : void
      {
         ...
         this.FMC_Scene = TUtilityReflection.CreateDisplayObjectInstance("MC_CornucopiaMySelected") as MovieClip;
      ...
      protected function UpdatePage() : void
      {
         ...
         this.FUIPage.TotalQuantity = this.FCornucopia.AllTickets.length;
         this.FUIPage.Update();
         _loc1_ = 0;
         while(_loc1_ < PAGE_COUNT)
         {
            ...
            if(_loc2_ < this.FCornucopia.AllTickets.length)
            {
               _loc3_.visible = true;
               _loc4_ = this.FCornucopia.AllTickets[_loc2_];
               ...
               _loc3_.TF_Num.text = TUtilityString.Format(this.FCornucopia.DescListNew[10],_loc5_,_loc4_.Count);
               if(_loc4_.Status == TTicketData.STATUS_NOT_OPEN)
               {
                  _loc3_.TF_Result.text = this.FCornucopia.DescListNew[8];
               }
               else if(_loc4_.Status == TTicketData.STATUS_NOT_WON)
               {
                  _loc3_.TF_Result.text = this.FCornucopia.DescListNew[9];
               }
               else
               {
                  _loc3_.TF_Result.text = this.FCornucopia.DescListNew[3 + _loc4_.Status] + ":" + _loc4_.Gold + STRING_COMMON.ITEMNAME_Gold;
               }
            }
            else
            {
      ...
      protected function ProcessorOnClose(param1:MouseEvent) : void
      {
         ...
         {
            this.FOnCloseUp(TProcessorCornucopia.WINDOW_MY_SELECTED);
         }
      }
