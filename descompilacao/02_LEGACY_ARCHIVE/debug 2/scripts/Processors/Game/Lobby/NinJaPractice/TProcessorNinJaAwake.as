package Processors.Game.Lobby.NinJaPractice
{
   ...
   public class TProcessorNinJaAwake extends TProcessorLobbyWindow
   {
      ...
      public function TProcessorNinJaAwake(param1:TUIComponent, param2:TProcessorNinJaPractice)
      {
         super(param1);
      ...
      {
         this.FReincarnationPanel = this.FNinJaPractice.MC_Awake;
      ...
      protected function UpdateAwakeDesc() : void
      {
         ...
         _loc1_ = SResourcesCore.ResourceBin.GetDatebase(CONST_DATEBASEVO.RESOURCEID_HeroTalent,_loc2_.Talent) as THeroTalent;
         _loc3_ = _loc1_.AwakeDesc.split("\\n");
         _loc4_ = 0;
         while(_loc4_ < Three)
         {
            TextField(this.FReincarnationPanel["TF_Awake_Describe_" + _loc4_]).text = _loc3_[_loc4_] ? _loc3_[_loc4_] : "";
            _loc4_++;
         }
         if(this.FCurHero.AwakeLevel == 1)
         {
            _loc5_ = TUtilityString.GetText(CONST_SYSTEMLANGUAGE.STRING_Awake_One);
         }
         else if(this.FCurHero.AwakeLevel == 2)
         {
            _loc5_ = TUtilityString.GetText(CONST_SYSTEMLANGUAGE.STRING_Awake_Two);
         }
         else if(this.FCurHero.AwakeLevel == 3)
         {
            _loc5_ = TUtilityString.GetText(CONST_SYSTEMLANGUAGE.STRING_Awake_Three);
         }
         else
         {
            _loc5_ = TUtilityString.GetText(CONST_SYSTEMLANGUAGE.STRING_Not_Awake);
         }
         this.FReincarnationbackScr.text = _loc5_;
      }
      
      protected function UpdateSlot() : void
      {
         var Fawakelevel:int = 0;
         var FHeroTalent:THeroTalent = null;
         var FBaseHero:TBaseHero = null;
         var FTmpObj:Object = null;
         var NeedCount:int = 0;
         var obj:Object = null;
         var i:int = 0;
         Fawakelevel = int(this.FCurHero.AwakeLevel);
         var IDTemplates:Vector.<uint> = new Vector.<uint>();
         var ItemCounts:Vector.<uint> = new Vector.<uint>();
         var Count:int = 2;
         FBaseHero = SResourcesCore.ResourceBin.GetDatebase(CONST_DATEBASEVO.RESOURCEID_BaseHero,this.FCurHero.Identifier) as TBaseHero;
         FHeroTalent = SResourcesCore.ResourceBin.GetDatebase(CONST_DATEBASEVO.RESOURCEID_HeroTalent,FBaseHero.Talent) as THeroTalent;
         if(FHeroTalent.AwakeEffect1 != "[]")
         {
            try
            {
               FTmpObj = FHeroTalent["AwakeEffectObject" + (Fawakelevel + 1)];
               NeedCount = int(FTmpObj.cost[0].amount);
            }
            catch(e:Error)
            {
               FTmpObj = FHeroTalent["AwakeEffectObject" + Fawakelevel];
               NeedCount = int(FTmpObj.cost[0].amount);
            }
            for each(obj in FTmpObj.cost)
            {
               IDTemplates.push(CONST_COMMON.GetItemIDByType(obj.type,obj.code,null));
               ItemCounts.push(obj.amount);
            }
            this.FSelectInventories.Clear();
            this.UnstreamizerInventoryReference.UnstreamizeGenerateInventoriesByIdentifiers(null,this.FSelectInventories,IDTemplates);
            i = 0;
            while(i < Count)
            {
               if(i < this.FSelectInventories.Count)
               {
                  this.SlotVector[i].Context = this.FSelectInventories.GetInventoryByIndex(i);
                  this["FTF_Material_Num" + i].text = this.getInventoryCountById(i) + "/" + ItemCounts[i];
                  this["FTF_Material_Num" + i].textColor = this.getInventoryCountById(i) >= ItemCounts[i] ? 26112 : 16711680;
               }
               else
               {
                  this.SlotVector[i].Context = null;
                  this["FTF_Material_Num" + i].text = "";
               }
               i++;
            }
         }
